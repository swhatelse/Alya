...
EOF
              ...
              pr If($kfl_limit_nsi == 1 => lambda{
                      if get_lang == FORTRAN then
                        ...
                        pr alpha[1] === Ternary( 1.0.to_var < Set(2.0.to_var, Real(:tmp, :vector_length => opts[:vector_length])) * ( Set(1.0.to_var, Real(:tmp, :vector_length => opts[:vector_length])) - beta[1] ), 1.0.to_var, Set(2.0.to_var, Real(:tmp, :vector_length => opts[:vector_length])) * ( Set(1.0.to_var, Real(:tmp, :vector_length => opts[:vector_length])) - beta[1] ) )
                      end
