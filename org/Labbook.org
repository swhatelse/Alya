...
***** Writing
      1. Intro
         The third approach consist in using accelerators, they are
         now part of the current HPC programming model
         porting some of Alya kernels on
         accelerators such as Knight Landing Xeon Phi. Accelerators
         are now part of the current programming model along with CPU.
         Our goal is to try to take advantage such hardware by
         exploiting for instance their vectorization capabilities,
         etc... We were given 2 matrix assembly kernels and in order
         to port these kernels we relied on BOAST, a meta programming
         framework. 
      2. Using BOAST
