...
***** Writing
      1. Intro
         ...
         meta-program his kernels using a dedicated language with
         orthogonal optimization and it can generate code in C, FORTRAN,
         CUDA and OpenCL. Thus, with this framework it is easy to
         generate different optimized versions of the same kernel to
         tune the performances according to the target platform. 
         ...
         - vectorize them in order to take advantage of the new AVX 512
           instructions set, 
         - test different optimization's strategies such as loop
      ...
      2. Using BOAST
         ...
         vector size of 2 and unrolling some loops we were able get at least
         a mean speedup of x1.5 and we hope reaching a proportional level of
         speedup on hardware supporting AVX 512.
         
         We are able to have speedup with only few optimization and
         ...
         #+begin_src R :results output graphics :file ./experiments/xp1/plots/speedup.png :exports both :width 800 :height 600 :session
           ggplot(df2, aes(x = factor(nest), y = speedup)) + 
               geom_bar(fill="grey38",stat = "identity", position="dodge") +
               ...
               theme(axis.text=element_text(size=14,color="black"), axis.title=element_text(size=16,face="bold"), plot.title = element_text(size=16,face="bold")) +
               theme_bw()
         #+end_src
