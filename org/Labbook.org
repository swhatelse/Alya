...
******* First loop
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest1(options)
            opts = {:vector_length => 1, :preprocessor => false, :nests => (1..11).to_a, :unroll => false, :inline => :included}
            opts.update(options)
            args = []
            args.push ndime = Int("ndime",  :dir => :in)
            args.push mnode = Int("mnode",  :dir => :in)
            ...
            args.push gpcar = Real("gpcar", :dir => :in,    :vector_length => opts[:vector_length], :dim => [Dim(ndime),Dim(mnode),Dim(pgaus)])
            args.push gpadv = Real("gpadv", :dir => :in,    :vector_length => opts[:vector_length], :dim => [Dim(ndime),Dim(pgaus)])
            args.push wgrgr = Real("wgrgr", :dir => :inout, :vector_length => opts[:vector_length], :dim => [Dim(pnode),Dim(pnode),Dim(pgaus)])
            args.push agrau = Real("agrau", :dir => :inout, :vector_length => opts[:vector_length], :dim => [Dim(pnode),Dim(pgaus)])
            ...
            if opts[:inline] == :included then
              ...
              inlined = opts[:inline] == :inline ? true : false 
              return Procedure("nest1", args, :inline => inlined, &block)
            end
          end
        #+END_SRC
******* Declaration
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          nests = []
          nests.push get_nest1(opts)

          if opts[:inline] == :call or opts[:inline] == :inlined then
            p = generate_boast_decl(opts, [pabdf_nsi,nests].flatten)
          else
          end
          ...
          @kernel.procedure = declare_procedure([pabdf_nsi])
          pr k_pabdf_nsi.procedure
          nests.each{|n| pr n } if opts[:inline] == :call or opts[:inline] == :inlined
          opn @kernel.procedure
            ...
            decl fact = Real('fact',           :vector_length => @opts[:vector_length], :dim => [Dim(9)], :allocate => allocate)
...
******* Initialisation
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          ...
          if opts[:nests].include? 1 then
            if nests[0].kind_of? Procedure then
              pr nests[0].call($ndime,$mnode,$pnode,$pgaus,$gpden,
