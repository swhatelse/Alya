...
**** Report
     ...
     Porting kernel using BOAST (meta programming framework):
     - Refactoring and targeting C,Fortran,Cuda,OpenCL
     - Testing different optimization strategies (unrolling, tiling, loop reordering)
     - Tuning the kernels according to the target architecture( size
       of vector, blocking size, unrolling factor,etc...)
* Sandbox :noexport:
** Fortran
*** Loop vectorization
    #+BEGIN_SRC fortran
