...
******* Fourth nest
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          ...
          ['i','j'].each{|e|
            code = <<EOF
              for_elauu[for_elauu.length] = For(#{e}dime,1,$ndime){
                pr #{e}dof[#{e}dime] === $ndime*#{e}node-$ndime+#{e}dime
                if e == 'i' then
          ...
          for_elbru = []
          2.times{|i|
            for_elbru[i] = For(idime,1,$ndime){
              pr $elrbu[idime,inode] === $elrbu[idime,inode] - fact[1] * gpveo[idime]
              pr $elrbu[idime,inode] === $elrbu[idime,inode] + fact[1] * $elvel[idime,inode,2] if i == 0 
            }
          }
          ...
          block1 = lambda{
            pr For(igaus,1,$pgaus){
              ...
              pr For(inode,1,$pnode){
                pr exp_fact1
                if opts[:unroll] then
                  pr for_elauu[0].unroll
                  pr for_elbru[0].unroll
                else
                  pr for_elauu[0]
                  pr for_elbru[0]
                end
          ...
          block2 = lambda{
            pr If($ndime == 2 => lambda{}, :else => block1)
          }      
          ...
          pr If($kfl_lumped == 1 => block2, $kfl_lumped == 2 => block3)  
          end
        #+END_SRC
******* Closing procedure
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
