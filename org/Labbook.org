        ...
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          class KSplit
            ...
            def initialize(options)
              ...
              @args = []
              @args.push @ndime         = Int("ndime", :dir => :in )
              @args.push @kfl_lumped    = Int("kfl_lumped",    :dir => :in)
              @args.push @mnode         = Int("mnode",         :dir => :in)
              @args.push @ntens         = Int("ntens",         :dir => :in)
              @args.push @kfl_duatss    = Int("kfl_duatss",    :dir => :in)
              @args.push @fact_duatss   = Int("fact_duatss",   :dir => :in)
              @args.push @kfl_stabi_nsi = Int("kfl_stabi_nsi", :dir => :in)
              @args.push @fvins_nsi     = Real("fvins_nsi",    :dir => :in)
              @args.push @fcons_nsi     = Real("fcons_nsi",    :dir => :in)
              @args.push @bemol_nsi     = Real("bemol_nsi",    :dir => :in)
              @args.push @kfl_regim_nsi = Int("kfl_regim_nsi", :dir => :in)
              @args.push @fvela_nsi     = Real("fvela_nsi",    :dir => :in, :dim => [Dim(3)])
              @args.push @kfl_rmom2_nsi = Int("kfl_rmom2_nsi", :dir => :in)
              @args.push @kfl_press_nsi = Int("kfl_press_nsi", :dir => :in)
              @args.push @kfl_p1ve2_nsi = Int("kfl_p1ve2_nsi", :dir => :in)
              @args.push @kfl_linea_nsi = Int("kfl_linea_nsi", :dir => :in)
              @args.push @kfl_confi_nsi = Int("kfl_confi_nsi", :dir => :in)
              @args.push @nbdfp_nsi     = Int("nbdfp_nsi",     :dir => :in)
              @args.push @kfl_sgsti_nsi = Int("kfl_sgsti_nsi", :dir => :in)
              @args.push @kfl_nota1_nsi = Int("kfl_nota1_nsi", :dir => :in)
              @args.push @kfl_limit_nsi = Int("kfl_limit_nsi", :dir => :in)
              @args.push @kfl_penal_nsi = Int("kfl_penal_nsi", :dir => :in)
              @args.push @penal_nsi     = Real("penal_nsi",    :dir => :in)
              @args.push @kfl_bubbl_nsi = Int("kfl_bubbl_nsi", :dir => :in)

              @args.push @pnode     = Int("pnode",             :dir => :in)
              @args.push @pgaus     = Int("pgaus",             :dir => :in)
              @args.push @gpden     = Real("gpden",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)])
              @args.push @gpvis     = Real("gpvis",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)])              
              @args.push @gppor     = Real("gppor",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)])
              @args.push @gpsp1     = Real("gpsp1",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)])
              @args.push @gpsp2     = Real("gpsp2",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)])
              @args.push @gpvol     = Real("gpvol",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)])
              @args.push @gpsha     = Real("gpsha",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@pnode),Dim(@pgaus)])
              @args.push @gpcar     = Real("gpcar",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@mnode),Dim(@pgaus)])
              @args.push @gpadv     = Real("gpadv",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@pgaus)])
              @args.push @gpvep     = Real("gpvep",            :dir => :inout , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@pgaus)])
              @args.push @gpgrp     = Real("gpgrp",            :dir => :inout , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@pgaus)])
              @args.push @gprhs     = Real("gprhs",            :dir => :inout , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@pgaus)])
              @args.push @gprhc     = Real("gprhc",            :dir => :inout , :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)])
              @args.push @gpvel     = Real("gpvel",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@pgaus),Dim()])
              @args.push @gpsgs     = Real("gpsgs",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@pgaus),Dim()])
              @args.push @elvel     = Real("elvel",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@pnode),Dim()])
              @args.push @elpre     = Real("elpre",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@pnode),Dim()])
              @args.push @elbub     = Real("elbub",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(1)])

              @args.push @wgrgr     = Real("wgrgr",            :dir => :out   ,:vector_length => @opts[:vector_length], :dim => [Dim(@pnode),Dim(@pnode),Dim(@pgaus)])
              @args.push @agrau     = Real("agrau",            :dir => :out   ,:vector_length => @opts[:vector_length], :dim => [Dim(@pnode),Dim(@pgaus)])

              # Matrices
              @args.push @elauu     = Real("elauu",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(@pnode*@ndime),Dim(@pnode*@ndime)])
              @args.push @elaup     = Real("elaup",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(@pnode*@ndime),Dim(@pnode)])
              @args.push @elapp     = Real("elapp",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(@pnode),Dim(@pnode)])
              @args.push @elapu     = Real("elapu",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(@pnode),Dim(@pnode*@ndime)])
              @args.push @elrbu     = Real("elrbu",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@pnode)])
              @args.push @elrbp     = Real("elrbp",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(@pnode)])
              # Others
              @args.push @dtinv_loc = Real("dtinv_loc",        :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(1)])
              @args.push @dtsgs     = Real("dtsgs",            :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(1)])
              @args.push @pbubl     = Int("pbubl",             :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(1)])
              @args.push @gpsha_bub = Real("gpsha_bub",        :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)])
              @args.push @gpcar_bub = Real("gpcar_bub",        :dir => :in    , :vector_length => @opts[:vector_length], :dim => [Dim(@ndime),Dim(@pgaus)])
              # Enrichement Element matrices
              @args.push @elauq     = Real("elauq",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(@pnode*@ndime),Dim(1)])
              @args.push @elapq     = Real("elapq",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(@pnode),Dim(1)])
              @args.push @elaqu     = Real("elaqu",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(1),Dim(@pnode*@ndime)])
              @args.push @elaqp     = Real("elaqp",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(1),Dim(@pnode)])
              @args.push @elaqq     = Real("elaqq",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(1),Dim(1)])
              @args.push @elrbq     = Real("elrbq",            :dir => :out   , :vector_length => @opts[:vector_length], :dim => [Dim(1)])
...
******* First loop
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest1(ndime,mnode,pnode,pgaus,gpden,gpcar,gpadv,wgrgr,agrau)
            inode = @inode
            jnode = @jnode
            igaus = @igaus
            ...
            block = lambda{
              ...
              decl inode,jnode,igaus unless @opts[:inline] == :included
            ...
            if @opts[:inline] == :included then
              return block
            else
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest1", [ndime,mnode,pnode,pgaus,gpden,gpcar,gpadv,wgrgr,agrau], :inline => inlined, &block)
            end
          end
        #+END_SRC
******* Second loop
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest2(ndime,mnode,pnode,pgaus,gpden,gpcar,gpsp2,gpvol,gpvis,dtinv_loc,
            ...
            args = [ndime,mnode,pnode,pgaus,gpden,gpcar,gpsp2,gpvol,gpvis,dtinv_loc,
                    gppor,gpsha,gpsp1_v,wgrgr,agrau,elauu] unless @opts[:inline] == :included
            ...
            block_fact8 = lambda{
              # pr fact[9] ===  gpden[igaus] * @k_pabdf_nsi.procedure.call(1) * dtinv_loc[1] + gppor[igaus]
              pr fact[9] ===  gpden[igaus] * dtinv_loc[1] + gppor[igaus]
            }
            ...
            main_block = lambda{
              decl fact,igaus,inode,jnode,idof,jdof unless @opts[:inline] == :included
              pr If(ndime == 2 => lambda{
                      pr for1[0]
                    }, else: lambda{
                      pr for1[1]
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest2", args, :inline => inlined, &main_block)
            end
          end
        #+END_SRC
******* Third nest
        ...
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest3(pgaus,pnode,mnode,ndime,gpvis,gpvol,gpcar,fvins_nsi,elauu)
            # locals
            igaus = @igaus
            idime = @idime
            jdime = @jdime
            ...
            main_block = lambda{ 
              decl igaus,idime,jdime,inode,jnode,fact,idofv,jdofv unless @opts[:inline] == :included
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest3", [pgaus,pnode,mnode,ndime,gpvis,gpvol,gpcar,fvins_nsi,elauu], :inline => inlined, &main_block)
            end
          end
        #+END_SRC
******* Fourth nest
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest4(ndime,pgaus,pnode,gpvol,gpden,dtinv_mod,gpsha,elvel,kfl_lumped,gpveo,elrbu,elauu)
            # locals 
            idime = @idime
            jdime = @jdime
            igaus = @igaus
...
EOF
            ...
            main_block = lambda{ 
              decl idime,jdime,igaus,inode,jnode,fact,idof,jdof unless @opts[:inline] == :included
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest4", [ndime,pgaus,pnode,gpvol,gpden,dtinv_mod,gpsha,elvel,kfl_lumped,gpveo,elrbu,elauu], :inline => inlined, &main_block)
            end
          end
        #+END_SRC
...
******* Fifth nest
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest5(ndime,mnode,pgaus,pnode,gpvol,gpsha,gpcar,elapu,elaup)
            # locals  
            igaus = @igaus
            inode = @inode
            jnode = @jnode
            ...
            main_block = lambda{
              decl igaus,inode,jnode,idime,jdime,idof,fact unless @opts[:inline] == :included
              if @opts[:unroll] then
                pr If(ndime == 2 => lambda{
                        pr for1[0]
                      }, else: lambda{
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest5", [ndime,mnode,pgaus,pnode,gpvol,gpsha,gpcar,elapu,elaup], :inline => inlined, &main_block)
            end
          end
        #+END_SRC
******* Sixth nest
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest6(kfl_stabi_nsi,pgaus,pnode,gpsp1_p,wgrgr,gpvol,elapp)
            # locals
            igaus = @igaus
            inode = @inode
            jnode = @jnode
            ...
            args = [kfl_stabi_nsi,pgaus,pnode,gpsp1_p,wgrgr,gpvol,elapp] unless @opts[:inline] == :included
            
            main_block = lambda{
              decl igaus,inode,jnode,fact unless @opts[:inline] == :included
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest6", args, :inline => inlined, &main_block)
            end
          end
        #+END_SRC
******* Seventh nest
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest7(pnode,pgaus,penal_nsi,gpvol,gpsha,elpre,elapp,elrbp)
            # locals
            fact = @fact
            igaus = @igaus
            inode = @inode
            ...
            main_block = lambda {
              decl fact,igaus,inode unless @opts[:inline] == :included
              pr For(igaus,1,pgaus){
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest7", [pnode,pgaus,penal_nsi,gpvol,gpsha,elpre,elapp,elrbp], :inline => inlined, &main_block)
            end
          end
        #+END_SRC        
******* Eighth nest
        ...
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest8(pgaus,ndime,pnode,elvel,agrau,gpsp1,kfl_limit_nsi,kfl_stabi_nsi,gpvep)
            # local
            igaus = @igaus
            idime = @idime
            inode = @inode
...
EOF
            ...
            main_block = lambda{
              decl igaus,idime,inode,c1,c2,c3,c4,beta,alpha unless @opts[:inline] == :included
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest8",[pgaus,ndime,pnode,elvel,agrau,gpsp1,kfl_limit_nsi,kfl_stabi_nsi,gpvep], :inline => inlined, &main_block)
            end
          end
        #+END_SRC        
******* Nineth nest
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest9(ndime,pgaus,gpsp1_p,gprhs,gpden,dtsgs,gpsp1_v,gpsgs,kfl_sgsti_nsi,kfl_stabi_nsi,gpgrp,gpvep)
            # local
            igaus = @igaus
            idime = @idime
            fact = @fact
           ...
           @dimensions.each{|dim|
            ...
            main_block = lambda{
              decl igaus,idime,fact,fact1_p unless @opts[:inline] == :included
              pr If(kfl_stabi_nsi == -1 => lambda{ 
                      if get_lang == FORTRAN then
                        pr gpgrp === 0.0 
                      elsif get_lang == C
...
EOF
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest9", [ndime,pgaus,gpsp1_p,gprhs,gpden,dtsgs,gpsp1_v,gpsgs,kfl_sgsti_nsi,kfl_stabi_nsi,gpgrp,gpvep], :inline => inlined, &main_block)
            end
          end
        #+END_SRC        
******* Tenth nest
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest10(mnode,ndime,pgaus,gpden,dtinv_mod,nbdfp_nsi,gpvel,pnode,gpvol,gpsha,agrau,gprhc,gpcar,gpvep,gpgrp,gprhs,elrbu,elrbp)
            # locals
            igaus = @igaus
            fact = @fact
            idime = @idime
              ...
              for_igaus[for_igaus.length] = For(igaus,1,pgaus){
                ...
                pr For(itime,2,nbdfp_nsi){
                  for_idime = For(idime,1,dim){
                    pr gprhs[idime,igaus] === gprhs[idime,igaus] - @k_pabdf_nsi.procedure.call(itime) * fact[5] * gpvel[idime,igaus,itime]
                  }
                  if @opts[:unroll] then
                    pr for_idime.unroll
                  else
            ...
            main_block = lambda{
              decl igaus,fact,idime,itime,inode unless @opts[:inline] == :included
              if @opts[:unroll] then
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              return Procedure("nest10",[mnode,ndime,pgaus,gpden,dtinv_mod,nbdfp_nsi,gpvel,pnode,gpvol,gpsha,agrau,gprhc,gpcar,gpvep,gpgrp,gprhs,elrbu,elrbp], :inline => inlined, :functions => [@k_pabdf_nsi.procedure], &main_block)
            end
          end
        #+END_SRC        
...
******* Eleventh nest
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          def get_nest11(mnode,pgaus,pnode,ndime,gpcar,gpvol,gpsha,gpcar_bub,pbubl,kfl_stabi_nsi,kfl_press_nsi,penal_nsi,elbub,gprhc,gpsha_bub,elauq,elaqu,elapq,elaqp,elaqq,elrbq)
            # locals
            fact = @fact
            igaus  = @igaus
            idof = @idof
            ...
            main_block = lambda{
              decl fact,igaus,idof,idime,inode unless @opts[:inline] == :included
...
EOF
            ...
            if @opts[:inline] == :included then
              ...
              inlined = @opts[:inline] == :inlined ? true : false 
              functions = get_lang == FORTRAN ? nil : [@p_maxval]
              return Procedure("nest11",[mnode,pgaus,pnode,ndime,gpcar,gpvol,gpsha,gpcar_bub,pbubl,kfl_stabi_nsi,kfl_press_nsi,penal_nsi,elbub,gprhc,gpsha_bub,elauq,elaqu,elapq,elaqp,elaqq,elrbq], :inline => inlined, :functions => functions, &main_block)
            end
          end
        #+END_SRC        
******* Fake pabdf_nsi
...
******* Declaration
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          ...
          # Local arrays
          @gpsp1_p   = Real("gpsp1_p",   :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)], :allocate => allocate)
          @gpsp1_v   = Real("gpsp1_v",   :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)], :allocate => allocate)
          @gpsp2_v   = Real("gpsp2_v",   :vector_length => @opts[:vector_length], :dim => [Dim(@pgaus)], :allocate => allocate)
          @c1        = Real("c1",        :vector_length => @opts[:vector_length], :dim => [Dim(1)], :allocate => allocate)
          @c2        = Real("c2",        :vector_length => @opts[:vector_length], :dim => [Dim(1)], :allocate => allocate)
          @c3        = Real("c3",        :vector_length => @opts[:vector_length], :dim => [Dim(1)], :allocate => allocate)
          @c4        = Real("c4",        :vector_length => @opts[:vector_length], :dim => [Dim(1)], :allocate => allocate)
          @alpha     = Real("alpha",     :vector_length => @opts[:vector_length], :dim => [Dim(1)], :allocate => allocate)
          @beta      = Real("beta",      :vector_length => @opts[:vector_length], :dim => [Dim(1)], :allocate => allocate)

          @gpveo     = Real("gpveo",     :vector_length => @opts[:vector_length], :dim => [Dim(3)], :allocate => allocate)
          @fact1_p   = Real("fact1_p",   :vector_length => @opts[:vector_length], :dim => [Dim(1)], :allocate => allocate)
          @dtinv_mod = Real("dtinv_mod", :vector_length => @opts[:vector_length], :dim => [Dim(1)], :allocate => allocate)
          @inode     = Int("inode")
          @jnode     = Int("jnode")
          @jdime     = Int("jdime")
          @idofv     = Int("idofv")
          ...
          @fact = Real('fact',           :vector_length => @opts[:vector_length], :dim => [Dim(9)], :allocate => allocate)

          @idof = Int("idof", :dim => [Dim(3)], :allocate => allocate)
          @jdof = Int("jdof", :dim => [Dim(3)], :allocate => allocate)

          nests = {}
          nests[:nest1] = get_nest1(*get_subroutine_args({:in => [@ndime,@mnode,@pnode,@pgaus,@gpden,
                                                                  @gpcar,@gpadv],
                                                          :inout => [@wgrgr,@agrau]})) if @opts[:nests].include? 1

          nests[:nest2] = get_nest2(*get_subroutine_args({:in => [@ndime,@mnode,@pnode,@pgaus,@gpden,
                                                                  @gpcar,@gpsp2,@gpvol,@gpvis,
                                                                  @dtinv_loc,@gppor,@gpsha,@gpsp1_v,
                                                                  @wgrgr,@agrau],
                                                          :inout => [@elauu]})) if @opts[:nests].include? 2

          nests[:nest3] = get_nest3(*get_subroutine_args({:in => [@pgaus,@pnode,@mnode,@ndime,@gpvis,@gpvol,
                                                                  @gpcar,@fvins_nsi],
                                                          :inout => [@elauu]}))if @opts[:nests].include? 3

          nests[:nest4] = get_nest4(*get_subroutine_args({:in => [@ndime,@pgaus,@pnode,@gpvol,@gpden,
                                                                  @dtinv_mod,@gpsha,@elvel,@kfl_lumped],
                                                          :inout => [@gpveo,@elrbu,@elauu]})) if @opts[:nests].include? 4

          nests[:nest5] = get_nest5(*get_subroutine_args({:in => [@ndime,@mnode,@pgaus,@pnode,@gpvol,@gpsha,
                                                                  @gpcar],
                                                          :inout => [@elapu,@elaup]})) if @opts[:nests].include? 5

          nests[:nest6] = get_nest6(*get_subroutine_args({:in => [@kfl_stabi_nsi,@pgaus,@pnode,@gpsp1_p,@wgrgr,@gpvol],
                                                          :inout => [@elapp]})) if @opts[:nests].include? 6

          nests[:nest7] = get_nest7(*get_subroutine_args({:in => [@pnode,@pgaus,@penal_nsi,@gpvol,@gpsha,@elpre],
                                                          :inout => [@elapp,@elrbp]})) if @opts[:nests].include? 7

          nests[:nest8] = get_nest8(*get_subroutine_args({:in => [@pgaus,@ndime,@pnode,@elvel,@agrau,@gpsp1,@kfl_limit_nsi,@kfl_stabi_nsi],
                                                          :inout => [@gpvep]})) if @opts[:nests].include? 8

          nests[:nest9] = get_nest9(*get_subroutine_args({:in => [@ndime,@pgaus,@gpsp1_p,@gprhs,@gpden,@dtsgs,@gpsp1_v,@gpsgs,@kfl_sgsti_nsi,@kfl_stabi_nsi],
                                                          :inout => [@gpgrp,@gpvep]})) if @opts[:nests].include? 9

          nests[:nest10] = get_nest10(*get_subroutine_args({:in => [@mnode,@ndime,@pgaus,@gpden,@dtinv_mod,@nbdfp_nsi,@gpvel,@pnode,@gpvol,@gpsha,@agrau,
                                                                    @gprhc,@gpcar,@gpvep,@gpgrp],
                                                            :inout => [@gprhs,@elrbu,@elrbp]})) if @opts[:nests].include? 10

          nests[:nest11] = get_nest11(*get_subroutine_args({:in => [@mnode,@pgaus,@pnode,@ndime,@gpcar,@gpvol,@gpsha,@gpcar_bub,@pbubl,@kfl_stabi_nsi,@kfl_press_nsi,
                                                                    @penal_nsi,@elbub,@gprhc,@gpsha_bub],
                                                            :inout => [@elauq,@elaqu,@elapq,@elaqp,@elaqq,@elrbq]})) if @opts[:nests].include? 11
          ...
          nests.values.each{|n| pr n } unless @opts[:inline] == :included
...
******* Calling subroutines
        #+BEGIN_SRC ruby  :tangle /tmp/alya.rb
          if @opts[:nests].include? 1 then
            if nests[:nest1].kind_of? Procedure then
              pr nests[:nest1].call(@ndime,@mnode,@pnode,@pgaus,@gpden,
                               @gpcar,@gpadv,@wgrgr,@agrau)
            else
              nests[:nest1].call
            end
          end
          if @opts[:nests].include? 2 then
            if nests[:nest2].kind_of? Procedure then
              pr nests[:nest2].call(@ndime,@mnode,@pnode,@pgaus,@gpden,
                               @gpcar,@gpsp2,@gpvol,@gpvis,@dtinv_loc,
                               @gppor,@gpsha,@gpsp1_v,@wgrgr,@agrau,@elauu)
            else
              nests[:nest2].call
            end
          ...
          if @opts[:nests].include? 3 then
            if nests[:nest3].kind_of? Procedure then
              pr nests[:nest3].call(@pgaus,@pnode,@mnode,@ndime,@gpvis,@gpvol,@gpcar,
                                    @fvins_nsi,@elauu)
            else
              nests[:nest3].call
            end
          ...
          if @opts[:nests].include? 4 then
            if nests[:nest4].kind_of? Procedure then
              pr nests[:nest4].call(@ndime,@pgaus,@pnode,@gpvol,@gpden,@dtinv_mod,
                                    @gpsha,@elvel,@kfl_lumped,@gpveo,@elrbu,@elauu)
            else
              nests[:nest4].call
            end
          end
          if @opts[:nests].include? 5 then
            if nests[:nest5].kind_of? Procedure then
              pr nests[:nest5].call(@ndime,@mnode,@pgaus,@pnode,@gpvol,@gpsha,@gpcar,@elapu,@elaup)
            else
              nests[:nest5].call
            end
          end
          if @opts[:nests].include? 6 then
            if nests[:nest6].kind_of? Procedure then
              pr nests[:nest6].call(@kfl_stabi_nsi,@pgaus,@pnode,@gpsp1_p,@wgrgr,@gpvol,@elapp)
            else
              nests[:nest6].call
            end
          end
          if @opts[:nests].include? 7 then
            if nests[:nest7].kind_of? Procedure then
              pr nests[:nest7].call(@pnode,@pgaus,@penal_nsi,@gpvol,@gpsha,@elpre,@elapp,@elrbp)
            else
              nests[:nest7].call
            end
          end
          if @opts[:nests].include? 8 then
            if nests[:nest8].kind_of? Procedure then
              pr nests[:nest8].call(@pgaus,@ndime,@pnode,@elvel,@agrau,@gpsp1,@kfl_limit_nsi,
                                    @kfl_stabi_nsi,@gpvep)
            else
              nests[:nest8].call
            end
          ...
          if @opts[:nests].include? 9 then
            if nests[:nest9].kind_of? Procedure then
              pr nests[:nest9].call(@ndime,@pgaus,@gpsp1_p,@gprhs,@gpden,@dtsgs,@gpsp1_v,@gpsgs,
                                    @kfl_sgsti_nsi,@kfl_stabi_nsi,@gpgrp,@gpvep)
            else
              nests[:nest9].call
            end
          end
          if @opts[:nests].include? 10 then
            if nests[:nest10].kind_of? Procedure then
              pr nests[:nest10].call(@mnode,@ndime,@pgaus,@gpden,@dtinv_mod,@nbdfp_nsi,@gpvel,@pnode,@gpvol,@gpsha,
                                     @agrau,@gprhc,@gpcar,@gpvep,@gpgrp,@gprhs,@elrbu,@elrbp)
            else
              nests[:nest10].call
            end
          end
          if @opts[:nests].include? 11 then
            if nests[:nest11].kind_of? Procedure then
              pr nests[:nest11].call(@mnode,@pgaus,@pnode,@ndime,@gpcar,@gpvol,@gpsha,@gpcar_bub,@pbubl,@kfl_stabi_nsi,
                                     @kfl_press_nsi,@penal_nsi,@elbub,@gprhc,@gpsha_bub,@elauq,@elaqu,@elapq,@elaqp,
                                     @elaqq,@elrbq)
            else
              nests[:nest11].call
            end
