        ...
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          ...
          EOF
            ...
            decl_ref = <<EOF
               ...
               real(rp),    intent(in)    :: gpden(#{opts[:vector_length]},pgaus)
               real(rp),    intent(in)    :: gpvis(#{opts[:vector_length]},pgaus)
               real(rp),    intent(in)    :: gppor(#{opts[:vector_length]},pgaus)
               real(rp),    intent(in)    :: gpsp1(#{opts[:vector_length]},pgaus)
               real(rp),    intent(in)    :: gpsp2(#{opts[:vector_length]},pgaus)
               real(rp),    intent(in)    :: gpvol(#{opts[:vector_length]},pgaus)
               real(rp),    intent(in)    :: gpsha(#{opts[:vector_length]},pnode,pgaus)
               real(rp),    intent(in)    :: gpcar(#{opts[:vector_length]},ndime,mnode,pgaus)
               real(rp),    intent(in)    :: gpadv(#{opts[:vector_length]},ndime,pgaus)
               real(rp),    intent(inout) :: gpvep(#{opts[:vector_length]},ndime,pgaus)
               real(rp),    intent(inout) :: gpgrp(#{opts[:vector_length]},ndime,pgaus)
               real(rp),    intent(inout) :: gprhs(#{opts[:vector_length]},ndime,pgaus)
               real(rp),    intent(inout) :: gprhc(#{opts[:vector_length]},pgaus)
               real(rp),    intent(in)    :: gpvel(#{opts[:vector_length]},ndime,pgaus,*)
               real(rp),    intent(in)    :: gpsgs(#{opts[:vector_length]},ndime,pgaus,*)
               real(rp),    intent(in)    :: elvel(#{opts[:vector_length]},ndime,pnode,*)
               real(rp),    intent(in)    :: elpre(#{opts[:vector_length]},pnode,*)
               real(rp),    intent(in)    :: elbub(#{opts[:vector_length]})
               ! Matrices
               real(rp),    intent(out)   :: elauu(#{opts[:vector_length]},pnode*ndime,pnode*ndime)
               real(rp),    intent(out)   :: elaup(#{opts[:vector_length]},pnode*ndime,pnode)
               real(rp),    intent(out)   :: elapp(#{opts[:vector_length]},pnode,pnode)
               real(rp),    intent(out)   :: elapu(#{opts[:vector_length]},pnode,pnode*ndime)
               real(rp),    intent(out)   :: elrbu(#{opts[:vector_length]},ndime,pnode)
               real(rp),    intent(out)   :: elrbp(#{opts[:vector_length]},pnode)
               ! Others
               real(rp),    intent(in)    :: dtinv_loc(#{opts[:vector_length]})
               real(rp),    intent(in)    :: dtsgs(#{opts[:vector_length]})
               integer(ip), intent(in)    :: pbubl(#{opts[:vector_length]})
               real(rp),    intent(in)    :: gpsha_bub(#{opts[:vector_length]},pgaus)
               real(rp),    intent(in)    :: gpcar_bub(#{opts[:vector_length]},ndime,pgaus)
               ! Enrichement Element matrices
               real(rp),    intent(out)   :: elauq(#{opts[:vector_length]},pnode*ndime,1)
               real(rp),    intent(out)   :: elapq(#{opts[:vector_length]},pnode,1)
               real(rp),    intent(out)   :: elaqu(#{opts[:vector_length]},1,pnode*ndime)
               real(rp),    intent(out)   :: elaqp(#{opts[:vector_length]},1,pnode)
               real(rp),    intent(out)   :: elaqq(#{opts[:vector_length]},1,1)
               real(rp),    intent(out)   :: elrbq(#{opts[:vector_length]},1)
               ! Local arrays
               real(rp)                   :: wgrgr(#{opts[:vector_length]},pnode,pnode,pgaus)
               real(rp)                   :: agrau(#{opts[:vector_length]},pnode,pgaus)
               real(rp)                   :: gpsp1_p(#{opts[:vector_length]},pgaus)
               real(rp)                   :: gpsp1_v(#{opts[:vector_length]},pgaus)
               real(rp)                   :: gpsp2_v(#{opts[:vector_length]},pgaus)
               real(rp)                   :: c1(#{opts[:vector_length]})
               real(rp)                   :: c2(#{opts[:vector_length]})
               real(rp)                   :: c3(#{opts[:vector_length]})
               real(rp)                   :: c4(#{opts[:vector_length]})
               real(rp)                   :: alpha(#{opts[:vector_length]})
               real(rp)                   :: beta(#{opts[:vector_length]})
               real(rp)                   :: fact0(#{opts[:vector_length]})
               real(rp)                   :: fact1(#{opts[:vector_length]})
               real(rp)                   :: fact2(#{opts[:vector_length]})
               real(rp)                   :: fact3(#{opts[:vector_length]})
               real(rp)                   :: fact4(#{opts[:vector_length]})
               real(rp)                   :: fact5(#{opts[:vector_length]})
               real(rp)                   :: fact6(#{opts[:vector_length]})
               real(rp)                   :: fact7(#{opts[:vector_length]})
               real(rp)                   :: fact8(#{opts[:vector_length]})
               real(rp)                   :: gpveo(#{opts[:vector_length]},3)
               real(rp)                   :: fact1_p(#{opts[:vector_length]})
               real(rp)                   :: dtinv_mod(#{opts[:vector_length]})
          ...
          EOF
            if opts[:preprocessor] then
              ...
              #define DEF_VECT ivect
              #else
              #define DEF_VECT 1:#{opts[:vector_length]}
              #endif
          EOF
            end
       #+END_SRC
...
******* Declaration
        #+BEGIN_SRC ruby :tangle /tmp/alya.rb
          ...
          opn p
            # Local arrays
            decl wgrgr     = Real("wgrgr",     :vector_length => opts[:vector_length], :dim => [Dim($pnode),Dim($pnode),Dim($pgaus)])
            decl agrau     = Real("agrau",     :vector_length => opts[:vector_length], :dim => [Dim($pnode),Dim($pgaus)])
