...
*** Parameters definition                                         :Technical:
    ...
    impact the declaration of all the procedures using this parameter. 
        #+BEGIN_SRC ruby :tangle ../src/boast/Common/Parameters.rb
          require 'BOAST'
          include BOAST
...
**** Class wrapper                                            :Technical:
     ...
     arguments when calling the subroutine effectively.
         #+BEGIN_SRC ruby :tangle ../src/boast/Common/subroutine.rb
...
***** Body
      ...
      =agrau= and =wgrgr=. Unlike in the reference implementation where
      the two unrolled versions of the =ndime= loop and wrapped into an
      if statement, we only generate the code for the corrected value
      of ndime. At the upper level the correct kernel for the
      corresponding value of =ndime= is chosen by a broker. 
      <<boast-nest1-body>>
...
***** Class                                                       :Technical:
      ...
      nest1 [[boast-nest1-body][<<boast-nest1-body>>]].  
...
***** Body
      ...
      according to the values that can take =ndime= or not.
